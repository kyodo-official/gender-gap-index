...
for (category in CATEGORIES) {
    ...
    for (subdir in paste0(DATA_DIR, category) |> list.dirs(full.names = TRUE)) {
        ...
        for (filename in paste0(subdir) |> list.files(pattern = "csv", full.names = TRUE)) {
            ...
            colnames(subcategory_ratio)[ncol(subcategory_ratio)] <- paste0(DATA_DIR, category, "/") |> gsub("", filename)
            colnames(subcategory_ratio)[ncol(subcategory_ratio)] <- gsub(".csv", "", colnames(subcategory_ratio)[ncol(subcategory_ratio)])
            colnames(subcategory_ratio)[ncol(subcategory_ratio)] <- paste0(subcategory, "/") |> gsub("", colnames(subcategory_ratio)[ncol(subcategory_ratio)])
        }
        ...
        category_ratio <- subcategory_ratio %*% col_weight |> apply(1, sum) |> cbind(category_ratio)
        colnames(category_ratio)[1] <- subcategory
        # ランキング(higher is better)の生成
        rankings <- -subcategory_ratio |> round(3) |> apply(2, rank, ties.method="min")
        
        # 都道府県データフレームに男女比データフレームを連結し、CSV形式で保存する
        prefecture |> cbind(subcategory_ratio |> round(3)) |> write.csv(paste0(RESULT_DIR, "csv/ratio_", subcategory, ".csv"), row.names = FALSE, quote = FALSE, fileEncoding = "UTF-8")
        prefecture |> cbind(rankings) |> write.csv(paste0(RESULT_DIR, "csv/rankings_", subcategory, ".csv"), row.names = FALSE, quote = FALSE, fileEncoding = "UTF-8")
    }    
    ...
    # ランキング(higher is better)の生成
    rankings <- -category_ratio |> round(3) |> apply(2, rank, ties.method="min")
    
    # 都道府県データフレームに男女比データフレームを連結し、CSV形式で保存する
    prefecture |> cbind(category_ratio |> round(3)) |> write.csv(paste0(RESULT_DIR, "csv/ratio_", category, ".csv"), row.names = FALSE, quote = FALSE, fileEncoding = "UTF-8")
    prefecture |> cbind(rankings) |> write.csv(paste0(RESULT_DIR, "csv/rankings_", category, ".csv"), row.names = FALSE, quote = FALSE, fileEncoding = "UTF-8")
}
...
prefecture |> cbind(index |> round(3)) |> write.csv(paste0(RESULT_DIR, "csv/index.csv"), row.names = FALSE, quote = FALSE, fileEncoding = "UTF-8")
